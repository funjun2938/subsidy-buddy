...
export default function HeaderNav({ email, isLoggedIn = false }: { email: string | null; isLoggedIn?: boolean }) {
  ...
  return (
    <>
      ...
      {/* 모바일 사이드 드로어 — document.body 로 포털.
          헤더의 backdrop-filter(.glass)가 position:fixed 의 기준(containing block)을
          헤더로 만들어 드로어/딤이 화면 전체를 못 덮던 문제를 회피한다. */}
      {mounted && createPortal(
        <>
          ...
          <button
            ...
            className={`fixed inset-0 z-[55] md:hidden bg-black/50 backdrop-blur-sm transition-opacity duration-200 cursor-default ${
          ...
          <aside
            aria-hidden={!open}
            className={`fixed top-0 right-0 z-[60] md:hidden h-full w-72 max-w-[82%] shadow-2xl border-l border-black/10 transition-transform duration-200 ease-out ${
